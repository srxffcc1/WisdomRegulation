...
public abstract class TwoWayLayoutManager extends LayoutManager {
    ...
    private void offsetChildren(int offset) {
        ...
        mLayoutStart += offset;
        mLayoutEnd += offset;
//        Log.v("TwoWayLayoutManager",offset+"");
    }
...
//        Log.v("TwoWayLayoutManager","delta:"+delta);
        final boolean cannotScrollForward = (firstPosition + childCount == state.getItemCount() &&
                mLayoutEnd <= end && delta >= 0);//是否能往上滚 大概意思

        final boolean cannotScrollBackward = (firstPosition == 0 &&
                mLayoutStart >= start && delta <= 0);//是否能往下滚 大概意思


//        Log.v("TwoWayLayoutManager",(firstPosition + childCount == state.getItemCount())+":"+mLayoutEnd+":"+start+":"+delta);
        Log.v("TwoWayLayoutManager",(firstPosition == 0)+":"+mLayoutStart+":"+start+":"+delta);
        Log.v("TwoWayLayoutManager",cannotScrollForward+":"+cannotScrollBackward+":"+delta);
