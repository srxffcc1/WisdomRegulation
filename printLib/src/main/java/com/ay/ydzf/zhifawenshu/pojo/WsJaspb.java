package com.ay.ydzf.zhifawenshu.pojo;

import com.ay.framework.annotation.ChineseName;
import com.ay.framework.core.pojo.BasePojo;

//@ChineseName("结案审批表")
public class WsJaspb extends BasePojo{
	@ChineseName("行政执法id")
	public String xzzfid;
	
	@ChineseName("执法文书名称")
	public String tname;

	@ChineseName("填报时间")
	public String tbsj;

	@ChineseName("企业ID")
	public String qyid;
	
	@ChineseName("地方")
	public String pcode;
	
	@ChineseName("字")
	public String ycode;
	
	@ChineseName("号")
	public String ncode;
	
	@ChineseName("案件名称")
	public String ajmc;
	
	@ChineseName("被处罚单位")
	public String bcfdw;
	
	@ChineseName("被处罚单位地址")
	public String bcfdwdz;
	
	@ChineseName("法定代表人")
	public String fddbr;
	
	@ChineseName("法定代表人职务")
	public String fddbrzw;
	
	@ChineseName("法定代表人邮编")
	public String fddbryb;
	
	@ChineseName("被处罚人")
	public String bcfr;
	
	@ChineseName("被处罚人年龄")
	public String bcfrnl;
	
	@ChineseName("被处罚人性别")
	public String bcfrxb;
	
	@ChineseName("被处罚人单位")
	public String bcfrdw;
	
	@ChineseName("被处罚人单位住址")
	public String bcfrdwzz;
	
	@ChineseName("被处罚人家庭住址")
	public String bcfrjtzz;
	
	@ChineseName("被处罚人联系电话")
	public String bcfrlxdh;
	
	@ChineseName("被处罚人邮编")
	public String bcfryb;
	
	@ChineseName("处理结果")
	public String cljg;
	
	@ChineseName("执行情况")
	public String zxqk;
	
	@ChineseName("承办人签名1")
	public String cbrqm_one;
	
	@ChineseName("承办人签名2")
	public String cbrqm_two;
	
	@ChineseName("承办人签名日期")
	public String cbrqmrq;
	
	@ChineseName("审核意见")
	public String shyj;
	
	@ChineseName("审核人签名")
	public String shrqm;
	
	@ChineseName("审核人签名日期")
	public String shrqmrq;
	
	@ChineseName("审批意见")
	public String spyj;
	
	@ChineseName("审批人签名")
	public String sprqm;
	
	@ChineseName("审批人签名日期")
	public String sprqmrq;
	
	@ChineseName("版本号")
	public String version;
	
	public String getAjmc() {
		return ajmc;
	}
	
	public String getBcfdw() {
		return bcfdw;
	}

	public String getBcfdwdz() {
		return bcfdwdz;
	}

	public String getBcfr() {
		return bcfr;
	}

	public String getBcfrdw() {
		return bcfrdw;
	}

	public String getBcfrdwzz() {
		return bcfrdwzz;
	}

	public String getBcfrjtzz() {
		return bcfrjtzz;
	}

	public String getBcfrlxdh() {
		return bcfrlxdh;
	}

	public String getBcfrnl() {
		return bcfrnl;
	}

	public String getBcfrxb() {
		return bcfrxb;
	}

	public String getBcfryb() {
		return bcfryb;
	}

	public String getCbrqm_one() {
		return cbrqm_one;
	}

	public String getCbrqm_two() {
		return cbrqm_two;
	}

	public String getCbrqmrq() {
		return cbrqmrq;
	}

	public String getCljg() {
		return cljg;
	}

	public String getFddbr() {
		return fddbr;
	}

	public String getFddbryb() {
		return fddbryb;
	}

	public String getFddbrzw() {
		return fddbrzw;
	}

	public String getNcode() {
		return ncode;
	}

	public String getPcode() {
		return pcode;
	}

	public String getQyid() {
		return qyid;
	}

	public String getShrqm() {
		return shrqm;
	}

	public String getShrqmrq() {
		return shrqmrq;
	}

	public String getShyj() {
		return shyj;
	}

	public String getSprqm() {
		return sprqm;
	}

	public String getSprqmrq() {
		return sprqmrq;
	}

	public String getSpyj() {
		return spyj;
	}

	public String getTbsj() {
		return tbsj;
	}

	public String getTname() {
		return tname;
	}

	public String getVersion() {
		return version;
	}

	public String getXzzfid() {
		return xzzfid;
	}

	public String getYcode() {
		return ycode;
	}

	public String getZxqk() {
		return zxqk;
	}

	public void setAjmc(String ajmc) {
		this.ajmc = ajmc;
	}

	public void setBcfdw(String bcfdw) {
		this.bcfdw = bcfdw;
	}

	public void setBcfdwdz(String bcfdwdz) {
		this.bcfdwdz = bcfdwdz;
	}

	public void setBcfr(String bcfr) {
		this.bcfr = bcfr;
	}

	public void setBcfrdw(String bcfrdw) {
		this.bcfrdw = bcfrdw;
	}

	public void setBcfrdwzz(String bcfrdwzz) {
		this.bcfrdwzz = bcfrdwzz;
	}

	public void setBcfrjtzz(String bcfrjtzz) {
		this.bcfrjtzz = bcfrjtzz;
	}

	public void setBcfrlxdh(String bcfrlxdh) {
		this.bcfrlxdh = bcfrlxdh;
	}

	public void setBcfrnl(String bcfrnl) {
		this.bcfrnl = bcfrnl;
	}

	public void setBcfrxb(String bcfrxb) {
		this.bcfrxb = bcfrxb;
	}

	public void setBcfryb(String bcfryb) {
		this.bcfryb = bcfryb;
	}

	public void setCbrqm_one(String cbrqm_one) {
		this.cbrqm_one = cbrqm_one;
	}

	public void setCbrqm_two(String cbrqm_two) {
		this.cbrqm_two = cbrqm_two;
	}

	public void setCbrqmrq(String cbrqmrq) {
		this.cbrqmrq = cbrqmrq;
	}

	public void setCljg(String cljg) {
		this.cljg = cljg;
	}

	public void setFddbr(String fddbr) {
		this.fddbr = fddbr;
	}

	public void setFddbryb(String fddbryb) {
		this.fddbryb = fddbryb;
	}

	public void setFddbrzw(String fddbrzw) {
		this.fddbrzw = fddbrzw;
	}

	public void setNcode(String ncode) {
		this.ncode = ncode;
	}

	public void setPcode(String pcode) {
		this.pcode = pcode;
	}

	public void setQyid(String qyid) {
		this.qyid = qyid;
	}

	public void setShrqm(String shrqm) {
		this.shrqm = shrqm;
	}

	public void setShrqmrq(String shrqmrq) {
		this.shrqmrq = shrqmrq;
	}

	public void setShyj(String shyj) {
		this.shyj = shyj;
	}

	public void setSprqm(String sprqm) {
		this.sprqm = sprqm;
	}

	public void setSprqmrq(String sprqmrq) {
		this.sprqmrq = sprqmrq;
	}

	public void setSpyj(String spyj) {
		this.spyj = spyj;
	}

	public void setTbsj(String tbsj) {
		this.tbsj = tbsj;
	}

	public void setTname(String tname) {
		this.tname = tname;
	}

	public void setVersion(String version) {
		this.version = version;
	}

	public void setXzzfid(String xzzfid) {
		this.xzzfid = xzzfid;
	}

	public void setYcode(String ycode) {
		this.ycode = ycode;
	}

	public void setZxqk(String zxqk) {
		this.zxqk = zxqk;
	}

	
	
}
