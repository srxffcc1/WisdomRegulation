package com.ay.ydzf.zhifawenshu.pojo;
import com.ay.framework.annotation.ChineseName;
import com.ay.framework.core.pojo.BasePojo;

//@ChineseName("行政强制审批表")
public class WsXzqzsp extends BasePojo {
	@ChineseName("行政执法id")
	public String xzzfid;
	@ChineseName("执法文书名称")
	public String tname;
	@ChineseName("填报时间")
	public String tbsj;
	@ChineseName("企业ID")
	public String qyid;
	@ChineseName("地方")
	public String pcode;
	@ChineseName("字")
	public String ycode;
	@ChineseName("号")
	public String ncode;
	
	
	@ChineseName("案件名称")
	public String ajmc;
	@ChineseName("公民姓名")
	public String gmxm;
	@ChineseName("公民性别")
	public String gmxb;
	@ChineseName("公民出生年月日")
	public String gmcxnyr;
	@ChineseName("公民联系电话")
	public String gmlxdh;
	@ChineseName("公民住址")
	public String gmzz;
	@ChineseName("法人名称")
	public String frmc;
	@ChineseName("法人")
	public String fr;
	@ChineseName("法人地址")
	public String frdz;
	@ChineseName("法人联系电话")
	public String frlxdh;
	@ChineseName("案件基本情况")
	public String ajjbqk;
	@ChineseName("承办人签名1")
	public String cbrqm1;
	@ChineseName("证件号1")
	public String zjh1;
	@ChineseName("承办人签名2")
	public String cbrqm2;
	@ChineseName("证件号2")
	public String zjh2;
	@ChineseName("承办人时间")
	public String cbrsj;
	@ChineseName("审核人签字")
	public String shrqz;
	@ChineseName("审核人时间")
	public String shrsj;
	
	@ChineseName("审批人签字")
	public String sprqz;
	@ChineseName("审批人时间")
	public String sprsj;
	
	@ChineseName("版本号")
	public String version;

	public String getSprqz() {
		return sprqz;
	}

	public void setSprqz(String sprqz) {
		this.sprqz = sprqz;
	}

	public String getSprsj() {
		return sprsj;
	}

	public void setSprsj(String sprsj) {
		this.sprsj = sprsj;
	}

	public String getXzzfid() {
		return xzzfid;
	}

	public void setXzzfid(String xzzfid) {
		this.xzzfid = xzzfid;
	}

	public String getTname() {
		return tname;
	}

	public void setTname(String tname) {
		this.tname = tname;
	}

	public String getTbsj() {
		return tbsj;
	}

	public void setTbsj(String tbsj) {
		this.tbsj = tbsj;
	}

	public String getQyid() {
		return qyid;
	}

	public void setQyid(String qyid) {
		this.qyid = qyid;
	}

	public String getPcode() {
		return pcode;
	}

	public void setPcode(String pcode) {
		this.pcode = pcode;
	}

	public String getYcode() {
		return ycode;
	}

	public void setYcode(String ycode) {
		this.ycode = ycode;
	}

	public String getNcode() {
		return ncode;
	}

	public void setNcode(String ncode) {
		this.ncode = ncode;
	}

	public String getAjmc() {
		return ajmc;
	}

	public void setAjmc(String ajmc) {
		this.ajmc = ajmc;
	}

	public String getGmxm() {
		return gmxm;
	}

	public void setGmxm(String gmxm) {
		this.gmxm = gmxm;
	}

	public String getGmxb() {
		return gmxb;
	}

	public void setGmxb(String gmxb) {
		this.gmxb = gmxb;
	}

	public String getGmcxnyr() {
		return gmcxnyr;
	}

	public void setGmcxnyr(String gmcxnyr) {
		this.gmcxnyr = gmcxnyr;
	}

	public String getGmlxdh() {
		return gmlxdh;
	}

	public void setGmlxdh(String gmlxdh) {
		this.gmlxdh = gmlxdh;
	}

	public String getGmzz() {
		return gmzz;
	}

	public void setGmzz(String gmzz) {
		this.gmzz = gmzz;
	}

	public String getFrmc() {
		return frmc;
	}

	public void setFrmc(String frmc) {
		this.frmc = frmc;
	}

	public String getFr() {
		return fr;
	}

	public void setFr(String fr) {
		this.fr = fr;
	}

	public String getFrdz() {
		return frdz;
	}

	public void setFrdz(String frdz) {
		this.frdz = frdz;
	}

	public String getFrlxdh() {
		return frlxdh;
	}

	public void setFrlxdh(String frlxdh) {
		this.frlxdh = frlxdh;
	}

	public String getAjjbqk() {
		return ajjbqk;
	}

	public void setAjjbqk(String ajjbqk) {
		this.ajjbqk = ajjbqk;
	}

	public String getCbrqm1() {
		return cbrqm1;
	}

	public void setCbrqm1(String cbrqm1) {
		this.cbrqm1 = cbrqm1;
	}

	public String getZjh1() {
		return zjh1;
	}

	public void setZjh1(String zjh1) {
		this.zjh1 = zjh1;
	}

	public String getCbrqm2() {
		return cbrqm2;
	}

	public void setCbrqm2(String cbrqm2) {
		this.cbrqm2 = cbrqm2;
	}

	public String getZjh2() {
		return zjh2;
	}

	public void setZjh2(String zjh2) {
		this.zjh2 = zjh2;
	}

	public String getCbrsj() {
		return cbrsj;
	}

	public void setCbrsj(String cbrsj) {
		this.cbrsj = cbrsj;
	}

	public String getShrqz() {
		return shrqz;
	}

	public void setShrqz(String shrqz) {
		this.shrqz = shrqz;
	}

	public String getShrsj() {
		return shrsj;
	}

	public void setShrsj(String shrsj) {
		this.shrsj = shrsj;
	}

	public String getVersion() {
		return version;
	}

	public void setVersion(String version) {
		this.version = version;
	}
}
