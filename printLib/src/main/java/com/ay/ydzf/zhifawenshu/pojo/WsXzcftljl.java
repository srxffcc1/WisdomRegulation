package com.ay.ydzf.zhifawenshu.pojo;

import com.ay.framework.annotation.ChineseName;
import com.ay.framework.core.pojo.BasePojo;

//@ChineseName("行政处罚集体讨论记录")
public class WsXzcftljl extends BasePojo{
	@ChineseName("行政执法id")
	public String xzzfid;
	@ChineseName("文书名称")
	public String tname;
	@ChineseName("填报时间")
	public String tbsj;
	@ChineseName("企业ID")
	public String qyid;
	@ChineseName("案件名称")
	public String ajmc;
	@ChineseName("讨论时间初")
	public String tlsjc;
	@ChineseName("讨论时间末")
	public String tlsjm;
	@ChineseName("地点")
	public String dd;
	@ChineseName("主持人")
	public String zcr;
	@ChineseName("汇报人")
	public String hbr;
	@ChineseName("记录人")
	public String jlr;
	@ChineseName("出席人员姓名及职务")
	public String cxry;
	@ChineseName("行政处罚集体讨论记录")
	public String tlnr;
	@ChineseName("讨论记录")
	public String tljl;
	@ChineseName("结论性意见")
	public String jlxyj;
	@ChineseName("出席人 签名")
	public String cxrqm;
	@ChineseName("行政处罚集体讨论记录")
	public String version;
	public String getAjmc() {
		return ajmc;
	}
	public String getCxrqm() {
		return cxrqm;
	}
	
	public String getCxry() {
		return cxry;
	}
	public String getDd() {
		return dd;
	}
	public String getHbr() {
		return hbr;
	}
	public String getJlr() {
		return jlr;
	}
	public String getJlxyj() {
		return jlxyj;
	}
	public String getQyid() {
		return qyid;
	}
	public String getTbsj() {
		return tbsj;
	}
	public String getTljl() {
		return tljl;
	}
	public String getTlnr() {
		return tlnr;
	}
	public String getTlsjc() {
		return tlsjc;
	}
	public String getTlsjm() {
		return tlsjm;
	}
	public String getTname() {
		return tname;
	}
	public String getVersion() {
		return version;
	}
	public String getXzzfid() {
		return xzzfid;
	}
	public String getZcr() {
		return zcr;
	}
	public void setAjmc(String ajmc) {
		this.ajmc = ajmc;
	}
	public void setCxrqm(String cxrqm) {
		this.cxrqm = cxrqm;
	}
	public void setCxry(String cxry) {
		this.cxry = cxry;
	}
	public void setDd(String dd) {
		this.dd = dd;
	}
	public void setHbr(String hbr) {
		this.hbr = hbr;
	}
	public void setJlr(String jlr) {
		this.jlr = jlr;
	}
	public void setJlxyj(String jlxyj) {
		this.jlxyj = jlxyj;
	}
	public void setQyid(String qyid) {
		this.qyid = qyid;
	}
	public void setTbsj(String tbsj) {
		this.tbsj = tbsj;
	}
	public void setTljl(String tljl) {
		this.tljl = tljl;
	}
	public void setTlnr(String tlnr) {
		this.tlnr = tlnr;
	}
	public void setTlsjc(String tlsjc) {
		this.tlsjc = tlsjc;
	}
	public void setTlsjm(String tlsjm) {
		this.tlsjm = tlsjm;
	}
	public void setTname(String tname) {
		this.tname = tname;
	}
	public void setVersion(String version) {
		this.version = version;
	}
	public void setXzzfid(String xzzfid) {
		this.xzzfid = xzzfid;
	}
	public void setZcr(String zcr) {
		this.zcr = zcr;
	}
	
	
}
