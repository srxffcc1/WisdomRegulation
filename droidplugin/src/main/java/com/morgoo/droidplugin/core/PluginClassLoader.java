...
public class PluginClassLoader extends DexClassLoader {
    private ClassLoader masterClassLoader;
    public PluginClassLoader(String apkfile, String optimizedDirectory, String libraryPath, ClassLoader masterClassLoader) {
        super(apkfile, optimizedDirectory, libraryPath, masterClassLoader.getParent());
        this.masterClassLoader =masterClassLoader;
    }
    ...
    protected Class<?> loadClass(String className, boolean resolve) throws ClassNotFoundException {
        ...
        try {
            return super.loadClass(className, resolve);//在插件中进行加载
        } catch (ClassNotFoundException e) {

        }
        try{
            return masterClassLoader.loadClass(className);//如果找不到就在宿主中再进行一次尝试
        }catch (ClassNotFoundException e){
