...
public class DiskCache implements Cache {
    ...
    public synchronized void clean() {
        ...
        mEntries.clear();
        mTotalSize = 0;
        KJHttpLoger.debug("disk Cache cleared.");
    }
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            ...
            KJHttpLoger.debug("%s: %s", file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                KJHttpLoger.debug("Unable to create cache dir %s",
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (!success) {
                KJHttpLoger.debug("Failed to write header for %s",
        ...
        if (!deleted) {
            KJHttpLoger.debug("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (HttpConfig.DEBUG) {
            KJHttpLoger.debug("Pruning old cache entries.");
        }
        ...
        if (HttpConfig.DEBUG) {
            KJHttpLoger.debug("pruned %d files, %d bytes, %d ms", prunedFiles,
    ...
    static class CacheHeader {
        ...
        public boolean writeHeader(OutputStream os) {
            try {
                ...
                KJHttpLoger.debug("%s", e.toString());
                return false;
            }
        }
